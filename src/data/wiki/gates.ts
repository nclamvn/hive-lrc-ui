...
export const gates: Milestone[] = [
  ...
  {
    id: "gate-d26", phase: "GATE D26",
    title: b("Labeled-root CRT campaign", "Chiến dịch CRT nghiệm-có-nhãn"),
    status: "partial", verify: "double", independence: "I2", date: "2026-07-19",
    summary: b(
      ...
      "Mọi đường tới nay đều trả giá nặng ở bước tái dựng, vì nó dựng lại các hàm đối xứng của tốc độ bình phương, sống dưới một ngưỡng rất cao, khoảng chặn tốc độ bình phương. D26 tấn công ngưỡng đó trực tiếp. Ý tưởng là nếu tìm kiếm duy trì một nhãn toàn cục đúng và cố định cho cả mười ba tốc độ, thì mỗi tốc độ riêng có thể ghim bằng định lý phần dư Trung Hoa ở ngưỡng thấp hơn nhiều, dựa trên chặn tốc độ thường thay vì bình phương của nó. Kỷ luật không thương lượng là danh tính người chạy, dấu, hoán vị và scale tổng thể được quyết một lần, cho cả tuple, ở cấp toàn cục, và không bao giờ canonicalize lại riêng tại mỗi prime hay mỗi denominator thời-gian-hữu-tỉ, vì đó chính là lỗi mà các gate trước phải gỡ. Gate phải chứng minh một tuple có-nhãn canonical duy nhất tồn tại và xác định rõ, ghim ngưỡng thấp chính xác cho tái dựng trực tiếp, dựng kiểm cục bộ giữ nhãn toàn cục, giữ trạng thái trong một master phân-tích prime-power, gộp các cut thời-gian-hữu-tỉ từ gate trước, và đo net state complexity thật, đếm mọi biến mới nhãn đưa vào. Một chiến dịch đầy đủ chỉ chạy khi cây trạng thái reachable chứng minh vừa giới hạn cố định. Không ngân sách compute lớn hơn được duyệt, và không có gì về giả thuyết được quyết bởi việc mở gate này. LRC(13) vẫn mở.",
    ),
    checks: [
      b("Maintain one correct GLOBAL labeling of the 13 speeds and CRT each V_i directly at a threshold based on B13 (much lower than the ~B13^2 squared-coefficient threshold). Prove the canonical labeled tuple exists and pin the exact direct-CRT threshold", "Giữ một nhãn TOÀN CỤC đúng cho 13 tốc độ và CRT từng V_i trực tiếp ở ngưỡng dựa trên B13 (thấp hơn nhiều ngưỡng hệ-số-bình-phương ~B13^2). Chứng minh tuple có-nhãn canonical tồn tại và ghim ngưỡng CRT-trực-tiếp chính xác"),
      b("Labels, signs, permutation and scale are GLOBAL objects fixed ONCE at the tuple level, NEVER re-canonicalized per prime or per rational-time denominator. Build a label-preserving local oracle and a prime-power factorized master, and integrate the D25 rational-time cuts", "Nhãn, dấu, hoán vị và scale là đối tượng TOÀN CỤC cố định MỘT LẦN ở cấp tuple, KHÔNG BAO GIỜ canonicalize lại theo prime hay theo denominator thời-gian-hữu-tỉ. Dựng local oracle giữ-nhãn và master phân-tích prime-power, tích hợp các cut thời-gian-hữu-tỉ D25"),
      b("Measure the net state complexity exactly, counting every new variable the labeling introduces; run a full campaign only if the reachable state tree provably fits the caps. No compute-budget increase; no GLOBAL-WITNESS; BASE-SURVIVOR escalates to D20-R. LRC(13) OPEN", "Đo net state complexity chính xác, đếm mọi biến mới nhãn đưa vào; chạy chiến dịch đầy đủ chỉ khi cây trạng thái reachable chứng minh vừa cap. Không tăng ngân sách compute; không GLOBAL-WITNESS; BASE-SURVIVOR leo thang tới D20-R. LRC(13) MỞ"),
      b("D26-PARTIAL (measured): the canonical labeled tuple and the direct coordinate CRT are proved and double-verified. The direct threshold is provably about half the squared-coefficient threshold, 969 bits versus 1946 bits, cutting the certified prime product from about 205 to about 109 primes, a real reduction. 723 of 723 corruption rejected. But the exact net-complexity ledger is decisive and negative: fixing one global labeling forces up to 13 factorial, about 6.2 billion, label assignments per accumulation prime, roughly 2^3546 labeling entropy, far above both the 977-bit threshold saving and the 2^42 compute cap. By the gate rule, a denominator that adds more unconstrained state than it removes is not progress, so the labeled campaign does not net-fit the caps. The reconstruction theorems stand; the campaign-fit closes negative. No BASE-SURVIVOR, no GLOBAL-WITNESS. LRC(13) OPEN", "D26-PARTIAL (đo được): tuple có-nhãn canonical và CRT tọa độ trực tiếp được chứng minh và kiểm kép. Ngưỡng trực tiếp chứng minh khoảng một nửa ngưỡng hệ-số-bình-phương, 969 bit so với 1946 bit, giảm tích prime chứng nhận từ khoảng 205 xuống khoảng 109 prime, một giảm thật. 723 trên 723 tấn công bị bác. Nhưng ledger net-complexity chính xác quyết định và âm: cố định một nhãn toàn cục buộc tới 13 giai thừa, khoảng 6.2 tỉ, gán nhãn mỗi prime tích lũy, khoảng 2^3546 entropy nhãn, vượt xa cả tiết kiệm ngưỡng 977 bit lẫn cap compute 2^42. Theo quy tắc gate, một denominator thêm nhiều trạng thái tự do hơn nó bỏ không phải tiến bộ, nên chiến dịch có-nhãn không net-vừa cap. Định lý tái dựng vẫn đứng; campaign-fit đóng âm. Không BASE-SURVIVOR, không GLOBAL-WITNESS. LRC(13) MỞ"),
    ],
    artifacts: 8,
  },
  {
    id: "gate-d27", phase: "GATE D27",
    title: b("Orbit-compressed unlabeled CRT: BLOCKED", "CRT không-nhãn nén-orbit: BLOCKED"),
    status: "blocked", verify: "double", independence: "I2", date: "2026-07-20",
    summary: b(
      "D26 proved the direct coordinate CRT threshold but measured the labeling obligation as a net-negative cost. D27 keeps the lower reconstruction threshold while removing the permanent per-prime labels, holding the global state as an unordered multiset of CRT root classes together with its stabilizer. Merging a new prime is done by enumerating matching orbits, that is transport matrices over the compatibility graph, rather than raw permutations. The orbit quotient is proved exact, with no false merge and no false split, and the sign-lift factor of 2^13 per prime is itemized for the first time. The decisive result is a certified obstruction, not a worst-case proxy: a single generic distinct-residue merge already produces 25,505,877,196,800 orbit children, about 2^44.5, which exceeds the frozen 2^42 state ceiling at merge depth one. The gate closes the forward orbit-enumerative unlabeled root-CRT architecture under the frozen caps. It does not rule out bidirectional or hybrid methods, and it does not decide the conjecture. LRC(13) remains open.",
      "D26 chứng minh ngưỡng CRT tọa độ trực tiếp nhưng đo nghĩa vụ gán nhãn là chi phí net âm. D27 giữ ngưỡng tái dựng thấp trong khi bỏ nhãn cố định theo prime, giữ trạng thái toàn cục dưới dạng đa-tập không thứ tự các lớp nghiệm CRT cùng stabilizer của nó. Ghép một prime mới thực hiện bằng liệt kê các orbit khớp, tức transport matrix trên đồ thị tương thích, thay vì hoán vị thô. Orbit quotient được chứng minh chính xác, không false merge và không false split, và hệ số sign-lift 2^13 mỗi prime được tách lần đầu. Kết quả quyết định là một obstruction chứng nhận, không phải proxy trường hợp xấu nhất: một merge generic residue-phân-biệt đơn đã sinh 25.505.877.196.800 orbit con, khoảng 2^44.5, vượt trần trạng thái 2^42 tại độ sâu ghép một. Gate đóng kiến trúc CRT-nghiệm không-nhãn liệt-kê-orbit tiến dưới cap đóng băng. Nó không loại phương pháp hai chiều hay hybrid, và không quyết giả thuyết. LRC(13) vẫn mở.",
    ),
    checks: [
      b("Orbit quotient theorem: child unordered CRT states correspond bijectively to transport matrices; raw matchings per orbit equal the exact multinomial. Proved and independently verified, no false merge or false split, repeated roots and non-free stabilizers handled", "Định lý orbit quotient: trạng thái CRT không thứ tự con tương ứng song ánh với transport matrix; số matching thô mỗi orbit bằng đa thức bội chính xác. Chứng minh và kiểm độc lập, không false merge/split, xử lý nghiệm lặp và stabilizer không tự do"),
      b("Shallow-zone theorem: below the certified threshold zone the available arithmetic and rational-time cuts are pairing-blind, so the reachable width is the exact product of orbit counts; one generic merge is 2^44.5, above the 2^42 ceiling at depth one", "Định lý shallow-zone: dưới vùng ngưỡng chứng nhận các cut số học và thời-gian-hữu-tỉ mù-với-ghép-cặp, nên độ rộng reachable là tích chính xác các đếm orbit; một merge generic là 2^44.5, trên trần 2^42 tại độ sâu một"),
      b("D27-BLOCKED (measured, certified at depth one). Verifiers V1 19/19 and V2 15/15 ACCEPT, corruption 255/255. The threshold theorem stands; the forward orbit-enumerative architecture does not fit the caps. No BASE-SURVIVOR, no GLOBAL-WITNESS. LRC(13) OPEN", "D27-BLOCKED (đo được, chứng nhận tại độ sâu một). Verifier V1 19/19 và V2 15/15 ACCEPT, corruption 255/255. Định lý ngưỡng vẫn đứng; kiến trúc liệt-kê-orbit tiến không vừa cap. Không BASE-SURVIVOR, không GLOBAL-WITNESS. LRC(13) MỞ"),
    ],
    artifacts: 8,
  },
  {
    id: "gate-d28", phase: "GATE D28",
    title: b("Certified bulge bridge: BLOCKED-BEFORE-BUILD", "Cầu vượt phình chứng nhận: BLOCKED-BEFORE-BUILD"),
    status: "blocked", verify: "single", independence: "I2", date: "2026-07-20",
    summary: b(
      "D28 tests whether the state-count region that blocked D27 can be crossed without enumerating its orbit children, examining a bidirectional symbolic meet and a coefficient-to-root switch at the design level before any production code. The bidirectional route is designed with product-form frontiers that defer pairing and sign, and the backward frontier is shown to be generable without knowing the target tuple, which removes a circularity risk. Its meet still leaves the local-cover choice product intact, because no frozen constraint couples cover choices across primes below the reconstruction zone. The switch route is sound only when the coefficient information already pins the root multiset, at which point it reduces to the earlier coefficient architecture and its known seeding cost. A label-free coefficient carrier pinning at 979 bits is recorded as a candidate finding. Both routes therefore have load-bearing obligations that measured upstream walls contradict, so no bridge implementable under the caps arises without a new coupling theorem. LRC(13) remains open.",
      "D28 kiểm liệu vùng số-lượng-trạng-thái đã chặn D27 có thể vượt qua mà không liệt kê orbit con hay không, xét một meet biểu tượng hai chiều và một chuyển hệ-số-sang-nghiệm ở mức thiết kế trước khi có code sản xuất. Đường hai chiều thiết kế với frontier dạng-tích hoãn ghép-cặp và dấu, và frontier lùi được chỉ ra sinh được mà không biết tuple đích, loại nguy cơ vòng lặp. Meet của nó vẫn để nguyên tích lựa-chọn-phủ cục bộ, vì không ràng buộc đóng băng nào ghép các lựa chọn phủ xuyên prime dưới vùng tái dựng. Đường switch chỉ sound khi thông tin hệ số đã ghim đa-tập nghiệm, khi đó nó về kiến trúc hệ số trước và chi phí seeding đã biết. Một carrier hệ số không-nhãn ghim ở 979 bit được ghi làm phát hiện ứng viên. Cả hai đường do đó có nghĩa vụ chịu lực mà các bức tường thượng nguồn đo được mâu thuẫn, nên không cầu nào triển khai được dưới cap xuất hiện mà thiếu một định lý ghép mới. LRC(13) vẫn mở.",
    ),
    checks: [
      b("Route A bidirectional meet: product-form frontiers defer pairing and sign; the backward frontier is generable without the target tuple. But the meet does not prune the local-cover product, so it is invalid under the frozen caps as specified", "Đường A meet hai chiều: frontier dạng-tích hoãn ghép-cặp và dấu; frontier lùi sinh được không cần tuple đích. Nhưng meet không cắt tích phủ cục bộ, nên không hợp lệ dưới cap đóng băng như đặc tả"),
      b("Route B coefficient-to-root switch: sound only at the pinning threshold, where it re-derives the earlier blocked architecture; a certified pre-pinning switch remains an open theorem. A 979-bit label-free coefficient carrier is a candidate", "Đường B chuyển hệ-số-sang-nghiệm: chỉ sound tại ngưỡng ghim, nơi nó dựng lại kiến trúc bị chặn trước; một switch trước-ghim chứng nhận vẫn là định lý mở. Một carrier hệ số không-nhãn 979 bit là ứng viên"),
      b("D28-BLOCKED-BEFORE-BUILD. Four missing theorems recorded as the reopening conditions. No production code written by decision. No BASE-SURVIVOR, no GLOBAL-WITNESS. LRC(13) OPEN", "D28-BLOCKED-BEFORE-BUILD. Bốn định lý còn thiếu được ghi làm điều kiện mở lại. Không viết code sản xuất theo quyết định. Không BASE-SURVIVOR, không GLOBAL-WITNESS. LRC(13) MỞ"),
    ],
    artifacts: 5,
  },
  {
    id: "gate-d29", phase: "GATE D29",
    title: b("Pre-reconstruction cross-prime coupling: PARTIAL", "Ghép xuyên-prime trước-tái-dựng: PARTIAL"),
    status: "partial", verify: "double", independence: "I2", date: "2026-07-20",
    summary: b(
      ...
      "D29 hỏi liệu bất biến không-nhãn, bất-biến-dấu nào ghép các lựa chọn phủ cục bộ xuyên prime trước tái dựng đầy đủ. Pha 1 chứng minh một thu gọn nội dung cục bộ: nội dung không-nhãn bất-biến-dấu đầy đủ của một phủ gấp cục bộ đúng bằng đa-tập bình phương, tương đương các dư hệ số đối xứng sơ cấp, nên mọi carrier cục bộ ứng viên đều qua một đối tượng hệ số. Pha 2 xác định ghép xuyên-prime khẳng định đầu tiên trong chương trình. Hệ số cao nhất bằng bình phương tích tốc độ, do đó một bình phương hoàn hảo bị chặn, và yêu cầu dư tái dựng của nó nhận một căn bậc hai bị chặn loại các trạng thái hệ số cục bộ mà thành viên độc lập nhận, với chứng chỉ chính xác và replay được, trên miền hữu hạn đầy đủ. Pha 3 khóa predicate này thành định lý: soundness, ngữ nghĩa một-scale-toàn-cục, cấu trúc modular-root chính xác, và một định lý đếm non-vacuity tổng quát. Bộ đánh giá chính xác quyết định nó là blocked-compute trong miền hữu ích dưới cap đóng băng, nên ghép được thiết lập toán học nhưng chưa dùng-được cho chiến dịch. LRC(13) vẫn mở.",
    ),
    checks: [
      b("Phase 1 PASS-REDUCTION: local content reduction proved; every label-free sign-invariant local observable factors through the coefficient object. V1 15/15, V2 10/10 independent, corruption 400/400", "Pha 1 PASS-REDUCTION: thu gọn nội dung cục bộ được chứng minh; mọi observable cục bộ không-nhãn bất-biến-dấu qua đối tượng hệ số. V1 15/15, V2 10/10 độc lập, corruption 400/400"),
      b("Phase 2 COUPLING-SURVIVOR: the top-coefficient square-lift predicate is sound and non-vacuous on complete domains, excluding 62 to 78 percent of admissible states with exact certificates, in the pre-reconstruction regime between B13 and B13 squared", "Pha 2 COUPLING-SURVIVOR: predicate square-lift hệ số cao nhất sound và non-vacuous trên miền đầy đủ, loại 62 tới 78 phần trăm trạng thái admissible với chứng chỉ chính xác, trong miền trước-tái-dựng giữa B13 và B13 bình phương"),
      b("Phase 3 PASS-Q13-THEOREM: soundness, one-global-scale semantics, modular-root structure and a general non-vacuity counting theorem proved and double-verified; the exact evaluator is blocked-compute in the useful regime. No BASE-SURVIVOR, no GLOBAL-WITNESS. LRC(13) OPEN", "Pha 3 PASS-Q13-THEOREM: soundness, ngữ nghĩa một-scale-toàn-cục, cấu trúc modular-root và một định lý đếm non-vacuity tổng quát được chứng minh và kiểm kép; bộ đánh giá chính xác blocked-compute trong miền hữu ích. Không BASE-SURVIVOR, không GLOBAL-WITNESS. LRC(13) MỞ"),
    ],
    artifacts: 24,
  },
  {
    id: "gate-d30", phase: "GATE D30",
    title: b("Certified bounded modular square root: PARTIAL", "Căn bậc hai modular bị chặn chứng nhận: PARTIAL"),
    status: "partial", verify: "double", independence: "I2", date: "2026-07-20",
    summary: b(
      ...
      "Ghép Pha 3 rút gọn phần tính toán hữu ích của nó về một bài toán chính xác: cho một modulus lẻ squarefree với phân tích đã biết, một dư là bình phương modulo mọi prime đã chọn, và một chặn, quyết định liệu một giá trị bị chặn bình phương thành dư đó. D30 nghiên cứu thuật toán chính xác và chứng chỉ replay được cho bài toán căn bậc hai modular bị chặn đó và giao một bộ đánh giá hybrid chứng nhận. Bộ đánh giá trả đúng một trong witness, empty-certified, open-incomplete, hoặc invalid-input, với ba đường chính xác, liệt kê nghiệm nhỏ, meet-in-the-middle, và q-scan, chọn bởi một dispatcher preflight xác định lấy đường rẻ nhất mà đếm chứng nhận vừa cap và ngược lại trả open tại preflight mà không materialize trạng thái. Chứng chỉ empty mang bản ghi hoàn chỉnh cấu trúc đầy đủ, không phải chỉ một hash, và một backend độc lập replay chúng. Trên miền hữu hạn đầy đủ mọi đường và dispatcher khớp brute force. Thể hiện hữu ích khoảng 109 prime trả open-incomplete tại preflight, vì các đường chính xác vượt cap ở đó. LRC(13) vẫn mở.",
    ),
    checks: [
      b("Sign-vector CRT and interval formulations proved: bounded modular square root is a signed modular subset-sum on the nonzero prime factors, with an exact global-sign quotient. Two independent exact backends agree on every complete domain", "Công thức sign-vector CRT và khoảng được chứng minh: căn bậc hai modular bị chặn là subset-sum modular có dấu trên các thừa số prime khác không, với thương global-sign chính xác. Hai backend chính xác độc lập khớp trên mọi miền đầy đủ"),
      b("Certified hybrid evaluator: enumeration, meet-in-the-middle and q-scan under a deterministic preflight dispatcher; the approximately 109-prime instance returns open-incomplete at preflight without materializing state. V1 12/12, V2 8/8 independent, corruption 400/400", "Bộ đánh giá hybrid chứng nhận: liệt kê, meet-in-the-middle và q-scan dưới dispatcher preflight xác định; thể hiện khoảng 109 prime trả open-incomplete tại preflight mà không materialize trạng thái. V1 12/12, V2 8/8 độc lập, corruption 400/400"),
      b("D30-PARTIAL, evaluator PASS-HYBRID-EVALUATOR within its measured operating envelope; the target regime remains blocked-compute for the implemented exact paths, which is not a lower bound for all algorithms. No campaign authorized. LRC(13) OPEN", "D30-PARTIAL, evaluator PASS-HYBRID-EVALUATOR trong envelope vận hành đo được; miền mục tiêu vẫn blocked-compute cho các đường chính xác đã triển khai, không phải chặn dưới cho mọi thuật toán. Không chiến dịch được duyệt. LRC(13) MỞ"),
    ],
    artifacts: 12,
  },
  {
    id: "gate-d31", phase: "GATE D31",
    title: b("Prime economy and the residue coset theorem: PARTIAL", "Kinh tế prime và định lý coset dư: PARTIAL"),
    status: "partial", verify: "double", independence: "I2", date: "2026-07-20",
    summary: b(
    ...
    checks: [
      b("Coset theorem proved: covers are closed under unit scaling and the top coefficient transforms by the twenty-sixth power, so the nonzero admissible residue set is a coset union; away from the exceptional class it is the full quadratic-residue subgroup, dense at (p-1)/2", "Định lý coset được chứng minh: phủ đóng dưới scale đơn vị và hệ số cao nhất biến đổi theo lũy thừa hai mươi sáu, nên tập dư admissible khác không là hợp coset; ngoài lớp ngoại lệ nó là toàn bộ nhóm con dư bậc hai, dày ở (p-1)/2"),
      b("Structured small-residue families are foreclosed at scale for the nonzero class: the minimum nonzero residue set is linear in the prime; a constant-size set requires the prime below 27. Sparse large-prime portfolios stay blocked under the dense model but are not refuted in general", "Họ dư-nhỏ cấu trúc bị đóng ở quy mô cho lớp khác không: tập dư khác không tối thiểu tuyến tính theo prime; tập kích-thước-hằng cần prime dưới 27. Danh mục prime lớn thưa vẫn blocked dưới mô hình dày nhưng không bị bác nói chung"),
      b("D31-PARTIAL with a proved PASS-THEORY coset result; the explicit Cartesian seeding is blocked-compute in the declared architecture; symbolic representation of the full constraint remains the open route. No BASE-SURVIVOR, no GLOBAL-WITNESS. LRC(13) OPEN", "D31-PARTIAL với kết quả coset PASS-THEORY được chứng minh; seeding Cartesian tường minh blocked-compute trong kiến trúc đã khai; biểu diễn biểu tượng của ràng buộc đầy đủ vẫn là đường mở. Không BASE-SURVIVOR, không GLOBAL-WITNESS. LRC(13) MỞ"),
    ],
    artifacts: 11,
  },
  {
    id: "gate-d32", phase: "GATE D32",
    title: b("Implicit realizability and the symbolic CEGAR kernel: PARTIAL", "Khả-thực-hiện ngầm và nhân CEGAR biểu tượng: PARTIAL"),
    status: "partial", verify: "double", independence: "I2", date: "2026-07-20",
    summary: b(
    ...
    checks: [
      b("Abstraction-refinement loop proved sound and exact on complete finite domains; each refinement preserves every governed tuple; the 34-state realizability pilot is established by a proved bijection, not by equal counts; a two-round empty case is certified on its domain", "Vòng trừu-tượng-tinh-chỉnh được chứng minh sound và chính xác trên miền hữu hạn đầy đủ; mỗi tinh chỉnh giữ mọi tuple governed; pilot khả-thực-hiện 34-trạng-thái được thiết lập bởi song ánh chứng minh, không bởi đếm bằng nhau; một trường hợp empty hai-vòng được chứng nhận trên miền của nó"),
      b("Symbolic kernel: local admissibility compresses to a node count linear in the prime count while representing the full product with zero Cartesian materialization; the coupling constraint construction materializes a bounded enumeration, moving the cost to preprocessing", "Nhân biểu tượng: admissibility cục bộ nén về số node tuyến tính theo số prime trong khi biểu diễn tích đầy đủ với materialization Cartesian bằng không; cấu trúc ràng buộc ghép materialize một liệt kê bị chặn, chuyển chi phí sang tiền xử lý"),
      b("D32-PARTIAL: a proved PASS-THEORY local-separability compression, with a representation-specific coupled-layer obstruction. V1 10/10, an independent second backend, corruption 560/560. No PASS-implicit-seeding, no campaign. No BASE-SURVIVOR, no GLOBAL-WITNESS. LRC(13) OPEN", "D32-PARTIAL: một nén tách-cục-bộ PASS-THEORY được chứng minh, với obstruction lớp-ghép đặc-thù-biểu-diễn. V1 10/10, một backend thứ hai độc lập, corruption 560/560. Không PASS-implicit-seeding, không chiến dịch. Không BASE-SURVIVOR, không GLOBAL-WITNESS. LRC(13) MỞ"),
    ],
    artifacts: 6,
  },
];
